...
arduino-build-cli.exe upload C:\Path\To\Sketch COM3 --full-upload
...
- **差分ページ書き込み**: 同じ接続中のボードへ直前に書き込んだフラッシュと
	比較し、変化した256バイトページだけを送信。COM構成変更またはdaemon再起動で
	自動的に無効化。完全一致なら接続・リセットも省略する。
	`--full-upload` で明示的に無効化可能。
- **キャッシュクリア**:
...
| 同一HEXの再アップロード | 約1.5秒 | 約20–30 ms (メモリキャッシュ) |
| 一部ページ変更の再アップロード | 約1.5秒 | 変更ページ数に比例 |
