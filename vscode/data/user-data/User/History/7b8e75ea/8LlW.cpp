...
namespace {
	...
	std::string formatDiagnosticLine(const std::string& raw) {
		...
		// kindとメッセージ部分を色付け
		const char* kindColor = isError ? "\033[1;31m" : isWarning ? "\033[1;33m" : "\033[1;36m";
		...
		std::ostringstream out;
		out << bold << location << reset          // ファイル:行:列 を太字
			<< ": " << kindColor << rest.substr(0, rest.find(':') + 1) << reset  // kind:
			<< " " << msgBody << "\n";            // メッセージ

		// 日本語ヒントを追加 (error/fatal error のみ、note は除く)
		if (isError || isWarning) {
			std::string hint = diagnosticHint(msgBody);
	...
	std::string colorizeOutput(const std::string& output) {
		...
		while (std::getline(iss, line)) {
			...
			// カウント (formatDiagnosticLine の中で種別判定済みなので簡易チェック)
		...
		if (errorCount > 0 || warningCount > 0) {
			oss << "\n\033[1;37m──────────────────────────────\033[0m\n";
			if (errorCount > 0)
				oss << "\033[1;31m  エラー  : " << errorCount << " 件\033[0m\n";
			if (warningCount > 0)
				oss << "\033[1;33m  警告    : " << warningCount << " 件\033[0m\n";
			oss << "\033[1;37m──────────────────────────────\033[0m\n";
		}
		return oss.str();
	}
