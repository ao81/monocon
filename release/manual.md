...
| `Disp& art(const char* a = "", const char* b = "", const char* c = "")` | 各桁について、1～8番のセグメントを文字列中の`.`で直感的に指定 |
| `Disp& off()` | 全桁を消灯 |
...
| `static constexpr uint8_t artPattern(const char* s)` | `art()`用文字列の1～8文字目をセグメントパターンへ変換 |
