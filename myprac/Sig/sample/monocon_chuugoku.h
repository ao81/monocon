...
protected:
	...
	void pollWith(uint8_t raw, uint32_t now) {
		...
		if (old == LOW && raw == HIGH) {
			if (fLtoh < UINT8_MAX) ++fLtoh;
			ltohEpoch = board_detail::loopEpoch;
		}

		if (old == HIGH && raw == LOW) {
			if (fHtol < UINT8_MAX) ++fHtol;
...
public:
	...
	bool ltoh() {
		if (!fLtoh) return false;
		--fLtoh;
		return true;
	}

	bool htol() {
		if (!fHtol) return false;
		--fHtol;
		return true;
	}

	uint8_t takeLtoh() {
		const uint8_t value = fLtoh;
		fLtoh = 0;
		return value;
	}

	uint8_t takeHtol() {
		const uint8_t value = fHtol;
		fHtol = 0;
		return value;
	}
	...
	uint8_t takeChange() {
		const uint16_t total =
			static_cast<uint16_t>(fHtol) + static_cast<uint16_t>(fLtoh);
		fHtol = 0;
		fLtoh = 0;
		return total > UINT8_MAX ? UINT8_MAX : static_cast<uint8_t>(total);
	}
};
...
public:
	Led()
		: color(0), opacity(255), acc(0), previousState(0xFF),
		blinking(false), blinkOn(false), blinkInterval(0), blinkNext(0) {
	}
	...
	void operator()(uint8_t newColor, uint8_t opacityPercent = 100) {
		...
		blinking = false;
		if (!wasBlinking && color == newColor && opacity == newOpacity) return;
		color = newColor;
		opacity = newOpacity;
	}

	void blink(uint8_t newColor, uint32_t intervalMs,
		uint8_t opacityPercent = 100) {
		if (intervalMs == 0) {
			(*this)(newColor, opacityPercent);
			return;
		}
		newColor &= 0x07;
		const uint8_t newOpacity = board_detail::percentToByte(
			clamp<uint8_t, int, int>(opacityPercent, 0, 100));
		if (blinking && color == newColor && opacity == newOpacity &&
		...
		blinkOn = true;
		blinkInterval = intervalMs;
		blinkNext = atomicMillis() + intervalMs;
	}
	...
	void serviceTick() {
		...
		if (blinking) {
			const uint32_t now = atomicMillis();
			if (static_cast<int32_t>(now - blinkNext) >= 0) {
				const uint32_t count =
					static_cast<uint32_t>(now - blinkNext) / blinkInterval + 1UL;
				if (count & 1UL) blinkOn = !blinkOn;
				blinkNext += count * blinkInterval;
			}
			if (!blinkOn) activeColor = 0;
		}

		uint8_t state;
		if (opacity == 0 || activeColor == 0) {
			state = 0;
		} else if (opacity == 255) {
			state = activeColor;
		} else {
...
private:
	...
	uint32_t intervalMs;
	uint16_t intervalTop;
	uint8_t intervalClock;
	volatile bool donePending;
	...
	static uint16_t timerTop(uint32_t ms, uint8_t& clock) {
		if (ms == 0) ms = 1;

		uint32_t prescaler;
		if (ms <= 262UL) {
			prescaler = 64UL;
			clock = _BV(CS41) | _BV(CS40);
		} else if (ms <= 1048UL) {
			prescaler = 256UL;
			clock = _BV(CS42);
		} else {
			if (ms > 4194UL) ms = 4194UL;
			prescaler = 1024UL;
			clock = _BV(CS42) | _BV(CS40);
		}

		uint32_t ticks =
			((F_CPU / prescaler) * ms + 500UL) / 1000UL;
	...
	void stopTimerLocked() {
		TCCR4B = _BV(WGM42);
		TIMSK4 &= static_cast<uint8_t>(~_BV(OCIE4A));
	}
	...
	void startTimerLocked() {
		...
		TIFR4 = _BV(OCF4A);
		TIMSK4 |= _BV(OCIE4A);
		TCCR4B = static_cast<uint8_t>(_BV(WGM42) | intervalClock);
	}
...
public:
	explicit Spm(Excitation mode = TWO_PHASE)
		...
		intervalClock(0),
		donePending(false) {
		intervalTop = timerTop(intervalMs, intervalClock);
		active = this;
	}
	...
	static inline void isrStep() {
		Spm* const p = active;
		if (!p) return;

		if (p->currentStep < p->targetStep) {
			p->stepCcw();
		} else if (p->currentStep > p->targetStep) {
			p->stepCw();
		}
	...
	void speed(uint32_t newIntervalMs) {
		if (newIntervalMs == 0) newIntervalMs = 1;
		if (newIntervalMs > 4194UL) newIntervalMs = 4194UL;
		if (intervalMs == newIntervalMs) return;

		uint8_t newClock;
		const uint16_t newTop = timerTop(newIntervalMs, newClock);

		ATOMIC_BLOCK(ATOMIC_RESTORESTATE) {
			intervalMs = newIntervalMs;
			intervalTop = newTop;
			intervalClock = newClock;
			startTimerLocked();
		}
	}

	void rela(float degree) {
		const int32_t steps = degreeToStep(degree);
		if (steps == 0) return;
		ATOMIC_BLOCK(ATOMIC_RESTORESTATE) {
			targetStep += steps;
			donePending = false;
	...
	void update(uint32_t newIntervalMs) {
		speed(newIntervalMs);
	}
