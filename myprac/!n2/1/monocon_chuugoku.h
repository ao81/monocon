...
public:
	...
	private:
		...
		Layer& overlay(Action action) {
			...
			for (uint8_t i = 0; i < 3; ++i) {
				owner_.pattern[i] =
					static_cast<uint8_t>(old[i] | owner_.pattern[i]);
			}
	...
	public:
		explicit Layer(Disp& owner)
			: owner_(owner) {
		}

		Layer& operator()(uint8_t a, uint8_t b, uint8_t c) {
			return overlay([&]() {
				owner_(a, b, c);
				});
		}

		Layer& s(const char* text) {
			return overlay([&]() {
				owner_.s(text);
				});
		}

		Layer& n(int x, bool zero = false, bool left = false) {
			return overlay([&]() {
				owner_.n(x, zero, left);
				});
		}
		...
		Layer& o(int oa, int ob, int oc) {
			owner_.o(oa, ob, oc);
			return *this;
		}

		Layer& o(int oa, int ob) {
			owner_.o(oa, ob);
			return *this;
		}
	...
	Layer n(int x, bool zero = false, bool left = false) {
		...
		if (neg) {
			content[ci++] = SEG_MINUS;
		}

		if (nd >= 3) {
			content[ci++] = seg[(value / 100) % 10];
		}

		if (nd >= 2) {
			content[ci++] = seg[(value / 10) % 10];
		}
		...
		if (zero && !neg) {
			...
			for (uint8_t i = 0; i < ci && i < 3; ++i) {
				out[i] = content[i];
			}
		} else {
			const uint8_t start =
				ci < 3 ? static_cast<uint8_t>(3 - ci) : 0;

			const uint8_t source =
				ci > 3 ? static_cast<uint8_t>(ci - 3) : 0;

			for (uint8_t i = source; i < ci; ++i) {
				out[start + i - source] = content[i];
			}
		}
	...
	Layer f(
		...
		if (neg) {
			...
			const int v = dot
				? static_cast<int>(bounded * 10.0 + 0.5)
	...
	Disp& o(int oa, int ob, int oc) {
		...
		for (uint8_t i = 0; i < 3; ++i) {
			opacity[i] =
				board_detail::percentToByte(values[i]);
		}
	...
	void serviceTick() {
		...
		for (uint8_t i = 0; i < 3; ++i) {
			if (opacity[i] == 0 || pattern[i] == 0) {
				...
				acc[i] =
					static_cast<uint8_t>(acc[i] + opacity[i]);

				out[i] =
					acc[i] < old ? pattern[i] : 0;
			}
		}
